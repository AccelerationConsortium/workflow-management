...
export class PrefectFlowConverter {
  ...
  public convertToPrefectFlow(workflow: Workflow): PrefectFlow {
    ...
    workflow.connections.forEach(connection => {
      const sourceTask = tasks.find(t => t.name.endsWith(connection.source));
      const targetTask = tasks.find(t => t.name.endsWith(connection.target));
