...
pub fn create_device(device_type: &str, device_id: String, config: Value) -> Result<Box<dyn DeviceExecutor>, DeviceError> {
    match device_type {
        "cva" => {
            let config: CVAConfig = serde_json::from_value(config).map_err(|e| DeviceError {
                device_id: device_id.clone(),
                error_code: DeviceErrorCode::InvalidParameter,
                message: format!("Invalid configuration: {}", e),
                source: Some(Box::new(e)),
            })?;
            Ok(Box::new(devices::cva::CVADevice::new(device_id, config)))
        }
        _ => Err(DeviceError {
            device_id: device_id.clone(),
            error_code: DeviceErrorCode::InvalidParameter,
            message: format!("Unsupported device type: {}", device_type),
            source: None,
        })
    }
} 
