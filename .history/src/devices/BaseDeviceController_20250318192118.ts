...
export abstract class BaseDeviceController implements DeviceController {
  ...
  constructor(protocol: CommunicationProtocol) {
  ...
  async executeOperation(operationId: string, params: Record<string, any>): Promise<void> {
    try {
      ...
      // Execute operation
      await this.protocol.sendCommand(operationId, params);
  ...
  async abortOperation(operationId: string): Promise<void> {
    try {
      await this.protocol.sendCommand('ABORT', { operationId });
  ...
  async setParameter(name: string, value: any): Promise<void> {
    try {
      await this.protocol.sendCommand('SET_PARAMETER', { name, value });
    } catch (error) {
      this.handleError(error as Error);
      throw error;
    }
  ...
  async getParameter(name: string): Promise<any> {
    try {
      const response = await this.protocol.sendCommand('GET_PARAMETER', { name });
