...
export class ParameterValidator implements ValidationRule {
  ...
  validate(context: ValidationContext): ValidationResult {
    ...
    for (const node of context.nodes.values()) {
      ...
      const paramDefs = getParameterDefinitions(node.type);
      
      // Skip validation if no parameter definitions found
      if (paramDefs.length === 0) {
        result.warnings.push({
          type: ValidationErrorType.INVALID_PARAMETER,
          severity: ValidationErrorSeverity.WARNING,
          message: `No parameter definitions found for node type: ${node.type}`,
          nodeId: node.id,
        });
        continue;
      }
