...
export interface WorkflowConfig extends WorkflowData {
  metadata: {
    lastModified: string;
    version: string;
  };
...
class WorkflowConfigService {
  ...
  public updateNode(nodeId: string, node: WorkflowNode): void {
    ...
    const existingNodeIndex = this.currentConfig!.nodes.findIndex(n => n.id === nodeId);
    if (existingNodeIndex >= 0) {
      this.currentConfig!.nodes[existingNodeIndex] = {
        ...node,
        id: nodeId
      };
    } else {
      this.currentConfig!.nodes.push({
        ...node,
        id: nodeId
      });
    }
  ...
  public removeNode(nodeId: string): void {
    ...
    this.currentConfig.nodes = this.currentConfig.nodes.filter(n => n.id !== nodeId);
    // Remove associated connections
    this.currentConfig.connections = this.currentConfig.connections.filter(
      (conn: any) => conn.source !== nodeId && conn.target !== nodeId
    );
    this.saveConfig();
  }
  ...
  public updateNodeParameters(nodeId: string, parameters: Record<string, any>): void {
    ...
    const node = this.currentConfig.nodes.find(n => n.id === nodeId);
    if (!node) {
  ...
  public updateConnection(connection: WorkflowConnection): void {
    ...
    const existingIndex = this.currentConfig!.connections.findIndex(
      (conn: any) => conn.source === connection.source && conn.target === connection.target
    );
    if (existingIndex >= 0) {
  ...
  public removeConnection(sourceId: string, targetId: string): void {
    ...
    this.currentConfig.connections = this.currentConfig.connections.filter(
      (conn: any) => !(conn.source === sourceId && conn.target === targetId)
    );
    this.saveConfig();
  }
