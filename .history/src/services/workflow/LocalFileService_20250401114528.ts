...
export class LocalFileService implements IFileService {
  ...
  async readFile(fileId: string, options?: FileReadOptions): Promise<{ data: any; metadata: FileMetadata }> {
    ...
    try {
      if (metadata.type === FileType.EXCEL) {
        data = await fs.readFile(filePath);
      } else {
        data = await fs.readFile(filePath, { encoding: options?.encoding || 'utf-8' });
      }
    } catch (error) {
    ...
    switch (metadata.type) {
      case FileType.JSON:
        ...
        break;
      case FileType.CSV:
        parsedData = parseCSV(data as string, formatOptions);
        break;
      case FileType.EXCEL:
        parsedData = parseExcel(data as Buffer, formatOptions);
        break;
      default:
        parsedData = data;
    }
  ...
  async writeFile(filename: string, data: any, options?: FileWriteOptions): Promise<FileMetadata> {
    ...
    const fileType = path.extname(filename).slice(1).toLowerCase() as FileType;
    
    // Validate file structure
    const validation = validateFileStructure(data, fileType);
    if (!validation.valid) {
    ...
    switch (fileType) {
      case FileType.JSON:
        ...
        break;
      case FileType.CSV:
        fileData = formatCSV(Array.isArray(data) ? data : [data], formatOptions);
        break;
      case FileType.EXCEL:
        fileData = formatExcel(
          typeof data === 'object' && !Array.isArray(data) ? data : { Sheet1: Array.isArray(data) ? data : [data] },
          formatOptions
        );
        break;
      default:
  ...
  async validateFile(file: any): Promise<{ valid: boolean; errors?: string[] }> {
    const fileType = path.extname(file.name).slice(1).toLowerCase() as FileType;
    return validateFileStructure(file.data, fileType);
  }
