...
export class WorkflowConverter {
   ...
   */
  public convertToPrefectFlow(workflow: Workflow): PrefectFlowDefinition {
    ...
    const dependencies = workflow.connections.map(conn => ({
      upstream_task: `${conn.source.nodeType}_${conn.source.nodeId}`,
      downstream_task: `${conn.target.nodeType}_${conn.target.nodeId}`
    }));
...
`;
  ...
  private validateWorkflow(workflow: Workflow): void {
    ...
    workflow.connections.forEach(conn => {
      const sourceExists = workflow.nodes.some(n => 
        n.id === conn.source.nodeId && n.type === conn.source.nodeType
      );
      const targetExists = workflow.nodes.some(n => 
        n.id === conn.target.nodeId && n.type === conn.target.nodeType
      );
  ...
  private hasCycles(workflow: Workflow): boolean {
    ...
    const dfs = (nodeId: string): boolean => {
      ...
      const outgoingConnections = workflow.connections.filter(
        conn => conn.source.nodeId === nodeId
      );
   ...
   * Check for disconnected nodes
   */
  private hasDisconnectedNodes(workflow: Workflow): boolean {
