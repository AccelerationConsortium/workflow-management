...
export const WorkflowStorage = {
  saveWorkflow: async (workflow: WorkflowData) => {
    try {
      console.log('WorkflowStorage.saveWorkflow - 原始输入:', workflow);
      ...
      console.log('WorkflowStorage.saveWorkflow - 处理后的工作流:', {
        id: workflowWithId.id,
        name: workflowWithId.name,
        nodesCount: workflowWithId.nodes.length,
        edgesCount: workflowWithId.edges.length
      });
