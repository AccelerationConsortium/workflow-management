...
export const BaseNode: React.FC<{ data: OperationNode }> = ({ data }) => {
  ...
  const handleFileUpload = async (inputId: string) => async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await fileUploadService.uploadFile(file, data.id, inputId);
                  ...
                  {data.inputs.map((input) => (
                    <div key={input.id} className="io-item">
                      ...
                      {uploadStatus[input.id]?.fileName && (
                        <div className="upload-info success">
                          Uploaded: {uploadStatus[input.id].fileName}
                        </div>
                      )}
                      {uploadStatus[input.id]?.error && (
                        <div className="upload-info error">
                          Error: {uploadStatus[input.id].error}
                        </div>
                      )}
                    </div>
                  ))}
