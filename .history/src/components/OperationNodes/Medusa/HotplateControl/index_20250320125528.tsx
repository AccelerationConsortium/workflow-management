...
export const HotplateControl: React.FC<HotplateControlProps> = ({ deviceId, onDataChange }) => {
  ...
  // Only update parameters when device state changes and in simulation mode
  useEffect(() => {
    if (state.data && isSimulating) {
      setParameters(prev => ({
        ...prev,
  ...
  }, [state.data, onDataChange, isSimulating]);

  const handleParameterChange = useCallback(async (field: string, value: number | boolean) => {
    if (!isSimulating) {
      console.log('Parameter changes are only allowed in simulation mode');
      return;
    }
  ...
  }, [parameters, sendCommand, isSimulating]);
  ...
  return (
    <div className="hotplate-control">
      {/* Basic parameter inputs always visible */}
      <div className="parameter-group">
