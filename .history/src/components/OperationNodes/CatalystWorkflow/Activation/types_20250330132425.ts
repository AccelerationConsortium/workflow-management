...
// Update PrimitiveConfig to include execution context and condition
export interface PrimitiveConfig {
  ...
  executionCondition?: {
    enabled_if?: string;  // e.g. "CVA.ecsa > 0.5"
    dependsOn?: string[]; // e.g. ["CVA"]
    timeout?: number;     // Optional timeout in milliseconds
    retryStrategy?: {
      maxAttempts: number;
      backoffMs: number;
...
export interface UnitOperationConfig {
  ...
  defaultEnabledPrimitives: string[];
  defaultParameters: Record<string, Record<string, ParameterValue>>;
