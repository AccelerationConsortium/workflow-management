...
impl PyDeviceExecutor {
    ...
    fn new(device_type: String, device_id: String, config: &PyDict) -> PyResult<Self> {
        let config_value = Python::with_gil(|py| {
            config.extract::<HashMap<String, Value>>(py)
        })?;
        let config = serde_json::to_value(config_value)?;
        let device = create_device(&device_type, device_id, config)?;
        ...
        let config_value = match config {
            Some(dict) => {
                let map = Python::with_gil(|py| {
                    dict.extract::<HashMap<String, Value>>(py)
                })?;
