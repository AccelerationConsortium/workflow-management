import { PrimitiveCategory } from '../components/OperationNodes/CatalystWorkflow/Activation/types';
...
// Update primitive interface to include category
interface Primitive {
  id: string;
  name: string;
  description: string;
...
export interface OperationNode {
  ...
  category: 'Sample Processing' | 'Analysis & Measurement' | 'Reaction Control' | 
             'Separation & Purification' | 'Data Acquisition' | 'Environment Control' | 'Test' | 'Medusa' | 'Catalyst Workflow';
  expanded?: boolean;
  icon?: string;
  parameters?: {
    name: string;
...
export const operationNodes: OperationNode[] = [
  ...
  },
  {
    type: 'Activation',
    label: 'Activation',
    description: 'Control device activation state',
    category: 'Medusa',
    expanded: true,
    specs: {
    ...
    parameters: [
      {
        ...
        type: 'string',
        default: 'manual',
        description: 'Operation mode (manual or timed)'
      },
      {
        name: 'activationTime',
        ...
        range: [0, 3600],
        default: 10,
        description: 'Duration of activation in timed mode'
      },
      {
        ...
        range: [0, 3600],
        default: 10,
        description: 'Duration of deactivation in timed mode'
      }
    ],
    inputs: [],
    outputs: [],
    ...
    return True`
      },
      {
        id: 'deactivate',
        name: 'Deactivate',
        description: 'Deactivate the device',
        order: 2,
        pythonCode: `def deactivate():
    device.deactivate()
    return True`
      },
      {
        id: 'run-timed',
        name: 'Run Timed Activation',
        description: 'Run timed activation cycle',
        order: 3,
        pythonCode: `def run_timed(activation_time: float = 10.0, deactivation_time: float = 10.0):
    device.activate()
        ...
        parameters: [
          {
            ...
            type: 'number',
            default: 10.0,
            description: 'Activation time in seconds'
          },
          {
            name: 'deactivation_time',
            type: 'number',
            default: 10.0,
            description: 'Deactivation time in seconds'
          }
        ]
      }
    ]
  },
  // Catalyst Workflow Nodes
  {
    type: 'Activation',
    label: 'Activation',
    description: 'Control device activation state',
    ...
    parameters: {
      ...
      activationTime: {
        type: 'number',
        description: 'Time to keep device activated',
        default: 0,
        unit: 'seconds'
      },
      deactivationTime: {
        type: 'number',
        description: 'Time to keep device deactivated',
        default: 0,
        unit: 'seconds'
      }
    },
      ...
      deactivate: {
        ...
        parameters: {
          delay: {
            ...
            type: 'number',
            default: 0,
            description: 'Delay before deactivation'
          }
        }
      },
      setMode: {
