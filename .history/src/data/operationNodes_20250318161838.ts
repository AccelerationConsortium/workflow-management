...
export interface OperationNode {
  ...
  category: 'Sample Processing' | 'Analysis & Measurement' | 'Reaction Control' | 
             'Separation & Purification' | 'Data Acquisition' | 'Environment Control' | 'Test' | 'Medusa';
  expanded?: boolean;
  parameters?: {
    name: string;
    ...
    inputs: [
      {
        ...
        label: 'Trigger',
        type: 'trigger',
        description: 'Start the heating/stirring process'
      }
    ],
    outputs: [
      {
    ...
    primitives: [
      {
        ...
        description: 'Set target temperature and start heating',
        order: 1,
        pythonCode: `def set_temperature(target_temp: float = 25.0, ramp_rate: float = 5.0):
    # Set target temperature with ramp rate
    return True`,
        parameters: [
          {
            name: 'target_temp',
      ...
      },
      {
        id: 'set-stirring',
        name: 'Set Stirring',
        description: 'Set stirring speed and start stirring',
        order: 2,
        pythonCode: `def set_stirring(speed: int = 0):
    # Set stirring speed
    return True`,
        parameters: [
          {
      ...
      {
        id: 'wait-temperature',
        name: 'Wait for Temperature',
        description: 'Wait until target temperature is reached',
        order: 3,
        pythonCode: `def wait_temperature(target_temp: float = 25.0, tolerance: float = 0.5):
    # Wait until temperature is within tolerance of target
    return True`,
        parameters: [
          {
            name: 'target_temp',
