...
pub fn create_device(device_type: &str, config: DeviceConfig) -> Result<Box<dyn DeviceExecutor>, DeviceError> {
    match device_type {
        "cva" => {
            let cva_config = match config {
                DeviceConfig::CVA(cfg) => cfg,
                _ => return Err(DeviceError::InvalidParameter {
                    reason: "配置类型不匹配".to_string(),
                    parameter: "config".to_string(),
                    valid_range: None,
                }),
            };
            Ok(Box::new(devices::cva::CVADevice::new(cva_config)))
        }
...
mod tests {
    ...
    async fn test_cva_device_creation() {
        ...
        let device = create_device("cva", config);
        assert!(device.is_ok());
    }
} 
