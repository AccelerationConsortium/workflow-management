...
def run_cv(params: Dict[str, Any]) -> ExecutionResult:
    ...
    --------
    ExecutionResult
        Contains measurement data and metadata for each scan rate
    """
    # Validate parameters
    errors = validate_params(params, [
        {"name": "start_voltage", "min": 0, "max": 5.0},
        {"name": "end_voltage", "min": 0, "max": 5.0},
        {"name": "scan_rates", "type": "list"},
    ...
    scan_rates = [float(rate) for rate in params["scan_rates"]]  # Convert to V/s
    ...
    for scan_rate in scan_rates:
        scan_rate = scan_rate / 1000  # Convert to V/s
        ...
        for cycle in range(cycles):
            ...
            while v <= end_v:
                ...
                scan_rate_labels.append(scan_rate * 1000)  # Convert back to mV/s for label
                v += step_size
                time += step_time
        ...
        # Create DataFrame for this scan rate
        data = pd.DataFrame({
            ...
            "cycle": cycle_numbers,
            "scan_rate": scan_rate_labels
        })
        
        all_data.append(data)
        
        # Calculate metadata for this scan rate
        metadata = {
            "scan_rate": float(scan_rate * 1000),  # Convert back to mV/s
    ...
    final_metadata = {
        "scan_rates": [m["scan_rate"] for m in all_metadata],
        "scan_rate_results": all_metadata,
