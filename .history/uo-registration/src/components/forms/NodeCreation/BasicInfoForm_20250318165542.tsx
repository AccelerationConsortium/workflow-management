    ...
    const subCategories = Object.entries(UnitOperationSubCategory).filter(([_, value]) => {
      switch (category) {
        ...
        case UnitOperationCategory.HEAT_TRANSFER:
          return ['HEAT_EXCHANGER', 'EVAPORATOR', 'CONDENSER'].includes(value);
        case UnitOperationCategory.MASS_TRANSFER:
          return ['ABSORPTION_COLUMN', 'ADSORPTION', 'MEMBRANE_SEPARATION'].includes(value);
        case UnitOperationCategory.FLUID_FLOW:
          return ['PUMPING', 'COMPRESSION', 'PIPING'].includes(value);
        case UnitOperationCategory.OTHERS:
          return ['MIXING', 'GRINDING', 'CUSTOM'].includes(value);
        default:
          return false;
      }
    });
